...
export default defineComponent({
  ...
  async run({ steps, $ }) {
    ...
    if (!dataToken) {
      throw new Error("Missing EIGHTX8_DATA_REQUEST_TOKEN.");
    }
    ...
    const statusCode = response?.["schedule-status"]?.status;

    if (statusCode === undefined || statusCode === null) {
      throw new Error(
        `Could not read schedule status from response: ${JSON.stringify(response)}`
      );
    }
